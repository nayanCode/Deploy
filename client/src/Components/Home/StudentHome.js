...
import {getStudentStatus} from '../../service/api';
...
const StudentHome = () => {
  ...
  useEffect(() =>  {
    setemail(matchEmail.id);
    const fetchData = async () => {
    let response = await getStudentStatus(matchEmail.id);
    if(!response) return ;
...
//      let response = await sentRequest(requestInitialValues);
...
//  }
    ...
    return(
      ...
      <Drawer variant="permanent" open={open}>
        ...
        <Divider />
        <List>
          {['Request', 'Status','View'].map((text, index) => (
            <ListItem key={text} onClick={() => RenderComponent(index)} disablePadding sx={{ display: 'block' }}>
              ...
              >
                ...
                >
                  {index  === 0 ? <InboxIcon /> :"" || index  === 1  ?<CheckCircleIcon/>:""|| index  === 2  ?<VisibilityRoundedIcon/>:"" }
                </ListItemIcon>
      ...
      <Box component="main" sx={{ flexGrow: 1, p:2 }}>
        ...
        {
         listClicked==='status' && <Status studentDetails={details}/>
       } 
       {
         listClicked==='view' && <View/>
       } 
      </Box>
        </Box>
    )
